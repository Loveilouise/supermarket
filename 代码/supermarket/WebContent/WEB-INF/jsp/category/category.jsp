 ...
 <div id="content"> <!-- Content start -->
      <div class="inner_content">
          <div class="widgets_area">
                <div class="row-fluid">
                    <div class="span12">
                      <form  action="${ctx}/category/findByMap.do" method="post">  
					    ...
					    <button type="submit" class="label label-warning" style="margin-left: 50px">查询</button>
					      <a href="${ctx}/category/add.do" class="label label-warning" style="float:right; margin:8px;">添加</a> 
                         </div>
                         </form>
                                    ...
                                    <tbody>
                                     <c:forEach items="${pagers.datas}" var="category" varStatus="l">
                                        <tr>
                                            ...
                                            <td>
                                            <c:if test="${category.isDelete==0}">
                                            <a class="btn" href="${ctx}/category/update.do?id=${category.id}" title="修改"><i class="icon-inbox"></i></a>
                                            &nbsp;&nbsp;&nbsp;&nbsp;
                                            <a class="btn"  href="${ctx}/category/delete.do?id=${category.id}" title="删除" onclick= "if(confirm( '是否删除！ ')==false)return   false; "><i class="icon-trash"></i></a>
                                           </c:if>
                                           <c:if test="${category.isDelete==1}">
                                                                                                                                       暂无
                                           </c:if>
      ...
      <pg:pager  url="${ctx}/category/findByMap.do" maxIndexPages="5" items="${pagers.total}"  maxPageItems="15" export="curPage=pageNumber" >
